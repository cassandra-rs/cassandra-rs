...
impl CassUuid {
    ...
    pub unsafe fn string(&self, output: *mut c_char) {cass_uuid_string(self.0,output)}
...
impl CassUuidGen {
    ...
    pub unsafe fn free(&self) {cass_uuid_gen_free(self.0)}
    pub unsafe fn time(&self, mut output: CassUuid) {cass_uuid_gen_time(self.0,&mut output.0)}
    pub unsafe fn random(&self, mut output: CassUuid) {cass_uuid_gen_random(self.0, &mut output.0)}
