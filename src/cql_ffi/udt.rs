...
impl DataType {
    ...
    pub fn new_from_existing(data_type: DataType) -> Self {
        unsafe { DataType(cass_data_type_new_from_existing(data_type.0)) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            ...
            CassandraError::build(cass_data_type_type_name(data_type.0,
        ...
        where S: Into<String>
    {
        unsafe {
            let type_name = CString::new(type_name.into()).unwrap();
            CassandraError::build(cass_data_type_set_type_name(data_type.0, type_name.as_ptr())).wrap(())
        }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let type_name = CString::new(type_name.into()).unwrap();
            CassandraError::build(cass_data_type_set_type_name_n(data_type.0,
                                                                 type_name.as_ptr(),
        ...
        where S: Into<String>
    {
        unsafe {
            ...
            CassandraError::build(cass_data_type_keyspace(data_type.0,
                                                          &mut (keyspace.as_ptr()),
        ...
        where S: Into<String>
    {
        unsafe {
            let keyspace = CString::new(keyspace.into()).unwrap();
            CassandraError::build(cass_data_type_set_keyspace(data_type.0, keyspace.as_ptr())).wrap(())
        }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let keyspace = CString::new(keyspace.into()).unwrap();
            CassandraError::build(cass_data_type_set_keyspace_n(data_type.0,
                                                                keyspace.as_ptr(),
        ...
        where S: Into<String>
    {
        unsafe {
            ...
            CassandraError::build(cass_data_type_class_name(data_type.0,
        ...
        where S: Into<String>
    {
        unsafe {
            let class_name = CString::new(class_name.into()).unwrap();
            CassandraError::build(cass_data_type_set_class_name(data_type.0, class_name.as_ptr())).wrap(())
        }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let class_name = CString::new(class_name.into()).unwrap();
            CassandraError::build(cass_data_type_set_class_name_n(data_type.0,
                                                                  class_name.as_ptr(),
    ...
    pub fn sub_data_type(data_type: DataType, index: u64) -> ConstDataType {
        unsafe { ConstDataType(cass_data_type_sub_data_type(data_type.0, index)) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_data_type_sub_type_name(data_type.0,
                                                               index,
                                                               &mut name.as_ptr(),
    ...
    pub fn add_sub_type(data_type: DataType, sub_data_type: DataType) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_data_type_add_sub_type(data_type.0, sub_data_type.0)).wrap(()) }
    }

    pub fn add_sub_type_by_name<S>(data_type: DataType, name: S, sub_data_type: DataType) -> Result<(), CassandraError>
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_data_type_add_sub_type_by_name(data_type.0, name.as_ptr(), sub_data_type.0))
                .wrap(())
        }
    }

    pub fn add_sub_value_type<S>(data_type: DataType, sub_value_type: ValueType) -> Result<(), CassandraError>
        where S: Into<String>
    {
        unsafe { CassandraError::build(cass_data_type_add_sub_value_type(data_type.0, sub_value_type as u32)).wrap(()) }
    }
}
...
impl UserType {
    pub fn new(data_type: ConstDataType) -> Self {
        unsafe { UserType(cass_user_type_new_from_data_type(data_type.0)) }
    }
    ...
    pub fn set_null(&mut self, index: u64) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_null(self.0, index)).wrap(()) }
    }

    pub fn set_int32(&mut self, index: u64, value: i32) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_int32(self.0, index, value)).wrap(()) }
    }
    ...
    pub fn set_int64(&mut self, index: u64, value: i64) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_int64(self.0, index, value)).wrap(()) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_user_type_set_int64_by_name(self.0, name.as_ptr(), value)).wrap(())
        }
    }

    pub fn set_float(&mut self, index: u64, value: f32) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_float(self.0, index, value)).wrap(()) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_user_type_set_float_by_name(self.0, name.as_ptr(), value)).wrap(())
        }
    }

    pub fn set_double(&mut self, index: u64, value: f64) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_double(self.0, index, value)).wrap(()) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_user_type_set_double_by_name(self.0, name.as_ptr(), value)).wrap(())
        }
    }

    pub fn set_bool(&mut self, index: u64, value: bool) -> Result<(), CassandraError> {
        unsafe {
            CassandraError::build(cass_user_type_set_bool(self.0, index, if value { 1u32 } else { 0u32 })).wrap(())
        }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            let value = CString::new(value.into()).unwrap();
            CassandraError::build(cass_user_type_set_string_by_name(self.0, name.as_ptr(), value.as_ptr())).wrap(())
        }
    }
    ...
    pub fn set_bytes(&mut self, index: u64, value: Vec<u8>) -> Result<(), CassandraError> {
        unsafe {
            CassandraError::build(cass_user_type_set_bytes(self.0, index, value.as_ptr(), value.len() as u64)).wrap(())
        }
    }
    ...
    pub fn set_uuid<S>(&mut self, index: u64, value: S) -> Result<(), CassandraError>
        where S: Into<Uuid>
    {
        unsafe { CassandraError::build(cass_user_type_set_uuid(self.0, index, value.into().0)).wrap(()) }
    }

    pub fn set_inet<S>(&mut self, index: u64, value: S) -> Result<(), CassandraError>
        where S: Into<Inet>
    {
        unsafe { CassandraError::build(cass_user_type_set_inet(self.0, index, value.into().0)).wrap(()) }
    }

    pub fn set_collection<S>(&mut self, index: u64, value: S) -> Result<(), CassandraError>
        where S: Into<Set>
    {
        unsafe { CassandraError::build(cass_user_type_set_collection(self.0, index, value.into().0)).wrap(()) }
    }
        ...
        where S: Into<String>
    {
        unsafe {
            let name = CString::new(name.into()).unwrap();
            CassandraError::build(cass_user_type_set_collection_by_name(self.0, name.as_ptr(), value.0)).wrap(())
        }
    }

    pub fn set_tuple(&mut self, index: u64, value: Tuple) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_tuple(self.0, index, value.0)).wrap(()) }
    }

    pub fn set_user_type(&mut self, index: u64, value: UserType) -> Result<(), CassandraError> {
        unsafe { CassandraError::build(cass_user_type_set_user_type(self.0, index, value.0)).wrap(()) }
    }
}
