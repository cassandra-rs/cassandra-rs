...
impl Statement {
    ...
    pub fn add_key_index(&mut self, index: u64) -> Result<&Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_add_key_index(self.0, index)).wrap(self) }
    }

    pub fn set_keyspace(&mut self, keyspace: String) -> Result<&Self, CassandraError> {
        unsafe {
            let keyspace = CString::new(keyspace).unwrap();
            CassandraError::build(cass_statement_set_keyspace(self.0, (keyspace.as_ptr()))).wrap(self)
        }
    }

    pub fn set_consistency(&mut self, consistency: Consistency) -> Result<&Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_set_consistency(self.0, consistency.0)).wrap(self) }
    }

    pub fn set_serial_consistency(&mut self, serial_consistency: Consistency) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_set_serial_consistency(self.0, serial_consistency.0)).wrap(self) }
    }

    pub fn set_paging_size(&mut self, page_size: i32) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_set_paging_size(self.0, page_size)).wrap(self) }
    }

    pub fn set_paging_state(&mut self, result: &CassandraResult) -> Result<&mut Self, CassandraError> {
        unsafe {
            try!(CassandraError::build(cass_statement_set_paging_state(self.0, result.0)).wrap(()));
            Ok(self)
        }
    }

    pub fn bind_null(&mut self, index: u64) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_null(self.0, index)).wrap(self) }
    }

    pub fn bind_int32(&mut self, index: u64, value: i32) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_int32(self.0, index, value)).wrap(self) }
    }

    pub fn bind_int64(&mut self, index: u64, value: i64) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_int64(self.0, index, value)).wrap(self) }
    }

    pub fn bind_float(&mut self, index: u64, value: f32) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_float(self.0, index, value)).wrap(self) }
    }

    pub fn bind_double(&mut self, index: u64, value: f64) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_double(self.0, index, value)).wrap(self) }
    }

    pub fn bind_bool(&mut self, index: u64, value: bool) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_bool(self.0, index, if value { 1 } else { 0 })).wrap(self) }
    }

    pub fn bind_string(&mut self, index: u64, value: &str) -> Result<&mut Self, CassandraError> {
        unsafe {
            let value = CString::new(value).unwrap();
            CassandraError::build(cass_statement_bind_string(self.0, index, value.as_ptr())).wrap(self)
        }
    }

    pub fn bind_bytes(&mut self, index: u64, value: Vec<u8>) -> Result<&mut Self, CassandraError> {
        unsafe {
            CassandraError::build(cass_statement_bind_bytes(self.0, index, value.as_ptr(), value.len() as u64))
                .wrap(self)
        }
    }

    pub fn bind_map(&mut self, index: u64, collection: Map) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_collection(self.0, index, collection.0)).wrap(self) }
    }
    ...
    pub fn bind_uuid(&mut self, index: u64, value: Uuid) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_uuid(self.0, index, value.0)).wrap(self) }
    }

    pub fn bind_inet(&mut self, index: u64, value: Inet) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_inet(self.0, index, value.0)).wrap(self) }
    }

    pub fn bind_user_type(&mut self, index: u64, value: UserType) -> Result<&mut Self, CassandraError> {
        unsafe { CassandraError::build(cass_statement_bind_user_type(self.0, index, value.0)).wrap(self) }
    }
    ...
    pub fn bind_int32_by_name(&mut self, name: &str, value: i32) -> Result<&mut Self, CassandraError> {
        unsafe {
            ...
            CassandraError::build(cass_statement_bind_int32_by_name(self.0, name.as_ptr(), value)).wrap(self)
        }
    }

    pub fn bind_int64_by_name(&mut self, name: &str, value: i64) -> Result<&mut Self, CassandraError> {
        unsafe {
            CassandraError::build(cass_statement_bind_int64_by_name(self.0,
    ...
    pub fn bind_float_by_name(&mut self, name: &str, value: f32) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_float_by_name(self.0, name.as_ptr(), value)).wrap(self)
        }
    }
    ...
    pub fn bind_double_by_name(&mut self, name: &str, value: f64) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_double_by_name(self.0, name.as_ptr(), value)).wrap(self)
        }
    }
    ...
    pub fn bind_bool_by_name(&mut self, name: &str, value: bool) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_bool_by_name(self.0, name.as_ptr(), if value { 1 } else { 0 }))
                .wrap(self)
        }
    }
    ...
    pub fn bind_bytes_by_name(&mut self, name: &str, mut value: Vec<u8>) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
    ...
    pub fn bind_uuid_by_name(&mut self, name: &str, value: Uuid) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_uuid_by_name(self.0, name.as_ptr(), value.0)).wrap(self)
        }
    }

    pub fn bind_inet_by_name(&mut self, name: &str, value: Inet) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_inet_by_name(self.0, name.as_ptr(), value.0)).wrap(self)
        }
    }
    ...
    pub fn bind_set_by_name(&mut self, name: &str, collection: Set) -> Result<&mut Self, CassandraError> {
        unsafe {
            let name = CString::new(name).unwrap();
            CassandraError::build(cass_statement_bind_collection_by_name(self.0, name.as_ptr(), collection.0))
                .wrap(self)
        }
    }
}
