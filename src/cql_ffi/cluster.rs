...
impl CassCluster {
    ...
    pub fn set_contact_points<S>(&mut self, contact_points: S) -> Result<&mut Self,CassError> where S: Into<String> {unsafe{
        let s = CString::new(contact_points.into()).unwrap();
        let err = CassError::build(cass_cluster_set_contact_points(self.0,s.as_ptr()));
        err.wrap(self)
    }}

    pub fn set_port(&mut self, port: i32) -> Result<&mut Self,CassError> {unsafe{
        CassError::build(cass_cluster_set_port(self.0,port)).wrap(self)
    }}

    pub fn set_ssl(&mut self, ssl: &mut CassSsl) -> &Self {unsafe{
        cass_cluster_set_ssl(self.0,ssl.0);
        self
    }}

    pub fn set_protocol_version(&mut self, protocol_version: i32) -> Result<&mut Self,CassError> {unsafe{
        CassError::build(cass_cluster_set_protocol_version(self.0,protocol_version)).wrap(self)
    }}

    pub fn set_num_threads_io(&mut self, num_threads: u32) -> Result<&Self,CassError> {unsafe{
    ...
    pub unsafe fn set_core_connections_per_host(&mut self, num_connections: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_core_connections_per_host(self.0,num_connections)).wrap(self)
    }

    pub unsafe fn set_max_connections_per_host(&mut self, num_connections: u32) -> Result<&mut Self,CassError> {
    ...
    pub unsafe fn set_max_concurrent_creation(&mut self, num_connections: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_max_concurrent_creation(self.0,num_connections)).wrap(self)
    }

    pub unsafe fn set_max_concurrent_requests_threshold(&mut self, num_requests: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_max_concurrent_requests_threshold(self.0,num_requests)).wrap(self)
    }

    pub unsafe fn set_max_requests_per_flush(&mut self, num_requests: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_max_requests_per_flush(self.0,num_requests)).wrap(self)
    }

    pub unsafe fn set_write_bytes_high_water_mark(&mut self, num_bytes: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_write_bytes_high_water_mark(self.0,num_bytes)).wrap(self)
    }

    pub unsafe fn set_write_bytes_low_water_mark(&mut self, num_bytes: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_write_bytes_low_water_mark(self.0,num_bytes)).wrap(self)
    }

    pub unsafe fn set_pending_requests_high_water_mark(&mut self, num_requests: u32) -> Result<&mut Self,CassError> {
        CassError::build(cass_cluster_set_pending_requests_high_water_mark(self.0,num_requests)).wrap(self)
    }

    pub unsafe fn set_pending_requests_low_water_mark(&mut self, num_requests: u32) -> Result<&mut Self,CassError> {
    ...
    pub fn set_load_balance_round_robin(&mut self) -> Result<&Self,CassError> {unsafe{
        cass_cluster_set_load_balance_round_robin(self.0);
        CassError::build(0).wrap(self)
    }}

    pub fn set_load_balance_dc_aware<S>(&mut self, local_dc: S,used_hosts_per_remote_dc: u32,allow_remote_dcs_for_local_cl: bool)-> Result<&Self,CassError> 
