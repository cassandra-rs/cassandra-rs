...
impl Drop for PreparedStatement {
    ...
    fn drop(&mut self) { unsafe { cass_prepared_free(self.0) } }
}

impl PreparedStatement {
    ///Creates a bound statement from a pre-prepared statement.
    pub fn bind(&self) -> Statement { unsafe { Statement(cass_prepared_bind(self.0)) } }
    ...
    pub fn parameter_data_type_by_name(&self, name: &str) -> ConstDataType {
        unsafe {
            ConstDataType(cass_prepared_parameter_data_type_by_name(self.0, CString::new(name).unwrap().as_ptr()))
        }
    }
}
